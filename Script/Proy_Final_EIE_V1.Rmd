...
fecha_corte_importacion <- as.Date("2024-12-01")   # Inicio importación GNI (tercer shock)


# Filtrar la base según nuestro schock
BD_pre <- BD_0 %>%
  filter(date < fecha_corte_importacion) #Opción 3
...
p1_imp <- plot_tendencias(BD_0, "grupo_IVIG_1_label",
                          "Productividad Industrial Nominal - Método 1 (Pesos iguales) - Importación GNI",
                          fecha_corte_importacion)  
p2_imp <- plot_tendencias(BD_0, "grupo_IVIG_2_label",
                          "Productividad Industrial Nominal - Método 2 (Pesos informados) - Importación GNI",
                          fecha_corte_importacion)  
p3_imp <- plot_tendencias(BD_0, "grupo_IVIG_3_label",
                          "Productividad Industrial Nominal - Método 3 (Var. Inversa) - Importación GNI",
                          fecha_corte_importacion)  

print(p1_imp); print(p2_imp); print(p3_imp)  
...
p_pre_covid_m1 <- plot_tendencias_pre(BD_0, "grupo_IVIG_1_label",
                                      "Productividad Industrial Nominal (Pre-COVID) - Método 1",
...
p_pre_gob_m1 <- plot_tendencias_pre(BD_0, "grupo_IVIG_1_label",
                                    "Productividad Industrial Nominal (Pre-Gobierno) - Método 1",
...
p_pre_covid_m2 <- plot_tendencias_pre(BD_0, "grupo_IVIG_2_label",
                                      "Productividad Industrial Nominal (Pre-COVID) - Método 2",
                                      fecha_corte_covid, meses_antes = 24)

# Para shock Gobierno (24 meses antes)
p_pre_gob_m2 <- plot_tendencias_pre(BD_0, "grupo_IVIG_2_label",
                                    "Productividad Industrial Nominal (Pre-Gobierno) - Método 2",
                                    fecha_corte_gobierno, meses_antes = 24)
...
p_pre_covid_m3 <- plot_tendencias_pre(BD_0, "grupo_IVIG_3_label",
                                      "Productividad Industrial Nominal (Pre-COVID) - Método 3",
                                      fecha_corte_covid, meses_antes = 24)

# Para shock Gobierno (24 meses antes)
p_pre_gob_m3 <- plot_tendencias_pre(BD_0, "grupo_IVIG_3_label",
                                    "Productividad Industrial Nominal (Pre-Gobierno) - Método 3",
                                    fecha_corte_gobierno, meses_antes = 24)

print(p_pre_covid_m3)
print(p_pre_gob_m3)

# Pre-tratamiento Importación
p_pre_imp_m1 <- plot_tendencias_pre(BD_0, "grupo_IVIG_1_label",
                                    "Productividad Industrial Nominal (Pre-Importación) - Método 1",
                                    fecha_corte_importacion, meses_antes = 24)  
p_pre_imp_m2 <- plot_tendencias_pre(BD_0, "grupo_IVIG_2_label",
                                    "Productividad Industrial Nominal (Pre-Importación) - Método 2",
                                    fecha_corte_importacion, meses_antes = 24)  
p_pre_imp_m3 <- plot_tendencias_pre(BD_0, "grupo_IVIG_3_label",
                                    "Productividad Industrial Nominal (Pre-Importación) - Método 3",
                                    fecha_corte_importacion, meses_antes = 24) 

print(p_pre_imp_m1); print(p_pre_imp_m2); print(p_pre_imp_m3)   
...
# Shock COVID (marzo 2020)
BD_0 <- BD_0 %>%
  mutate(
    tiempo_rel_covid = interval(date, fecha_corte_covid) %/% months(1),
    periodo_covid = as.integer(tiempo_rel_covid)
  )

# Shock Gobierno (agosto 2022)
BD_0 <- BD_0 %>%
  mutate(
    tiempo_rel_gob = interval(date, fecha_corte_gobierno) %/% months(1),
    periodo_gob = as.integer(tiempo_rel_gob)
  )

# Shock Importación (dic 2024)
BD_0 <- BD_0 %>%                                                
  mutate(                                                        
    tiempo_rel_imp = interval(date, fecha_corte_importacion) %/% months(1),  
    periodo_imp = as.integer(tiempo_rel_imp)                                  
  )     

# Verificar distribución por los shocks
cat("Rango de períodos COVID:", 
    range(BD_0$periodo_covid, na.rm = TRUE), "\n")
cat("Rango de períodos Gobierno:", 
    range(BD_0$periodo_gob, na.rm = TRUE), "\n\n")
...
ventana_covid <- c(-36, 12)    # 12 meses antes, 36 meses después
ventana_gob <- c(-36, 12)      # 12 meses antes, 36 meses después
ventana_imp <- c(-36, 4)       # 6 meses antes, 6 meses después

BD_event_covid <- BD_0 %>%
  filter(periodo_covid >= ventana_covid[1] & 
         periodo_covid <= ventana_covid[2])

BD_event_gob <- BD_0 %>%
  filter(periodo_gob >= ventana_gob[1] & 
         periodo_gob <= ventana_gob[2])

BD_event_imp <- BD_0 %>%                                         
  filter(periodo_imp >= ventana_imp[1] &                          
         periodo_imp <= ventana_imp[2])                           

cat("Observaciones para Event Study COVID:", nrow(BD_event_covid), "\n")
cat("Observaciones para Event Study Gobierno:", nrow(BD_event_gob), "\n\n")
...
# EVENT STUDY - SHOCK COVID
# Usar período -1 como referencia (omitido)
modelo_event_covid <- feols(
  ind_prod_nominal ~ i(periodo_covid, IVIG_3_dt, ref = -1) + 
                     + cobertura + 
...
# EVENT STUDY - SHOCK GOBIERNO
modelo_event_gob <- feols(
  ind_prod_nominal ~ i(periodo_gob, IVIG_3_dt, ref = -1) + 
                     cobertura +  
                     log(población) + log(SGP) |
                     departamento + ym,
  data = BD_event_gob,
...
modelo_event_imp <- feols(                                        
  ind_prod_nominal ~ i(periodo_imp, IVIG_3_dt, ref = -1) +        
                     cobertura +                                   
                     log(población) + log(SGP) |                  
                     departamento + ym,                            
  data = BD_event_imp,                                            
...
# Extraer coeficientes
coefs_covid <- extraer_coefs_event(modelo_event_covid, "periodo_covid")
coefs_gob <- extraer_coefs_event(modelo_event_gob, "periodo_gob")
coefs_imp <- extraer_coefs_event(modelo_event_imp, "periodo_imp")   


cat("Coeficientes extraídos:\n")
cat("  COVID: N períodos =", nrow(coefs_covid), "\n")
cat("  Gobierno: N períodos =", nrow(coefs_gob), "\n\n")
...
# Crear gráficos
p_event_covid <- plot_event_study(
  coefs_covid,
  "Event Study: Efecto Dinámico del Shock COVID-19",
  "Diciembre 2021"
)

p_event_gob <- plot_event_study(
  coefs_gob,
  "Event Study: Efecto Dinámico del Cambio de Gobierno",
  "Agosto 2022"
)
...
test_pre_covid <- coefs_covid %>%
  filter(periodo < -1) %>%  # Excluir referencia
  summarise(
    n_coefs = n(),
...
cat("SHOCK COVID:\n")
cat("  N coeficientes pre-shock:", test_pre_covid$n_coefs, "\n")
cat("  N significativos:", test_pre_covid$n_sig, "\n")
cat("  Máximo coeficiente (abs):", round(test_pre_covid$max_abs_coef, 3), "\n")
if(test_pre_covid$n_sig == 0) {
...
cat("SHOCK GOBIERNO:\n")
cat("  N coeficientes pre-shock:", test_pre_gob$n_coefs, "\n")
cat("  N significativos:", test_pre_gob$n_sig, "\n")
cat("  Máximo coeficiente (abs):", round(test_pre_gob$max_abs_coef, 3), "\n")
if(test_pre_gob$n_sig == 0) {
  cat("Ningún coeficiente pre-shock significativo → Tendencias paralelas VÁLIDAS\n\n")
} else {
  cat("Algunos coeficientes pre-shock significativos → Posible violación\n\n")
}
...
# ------------------------------------------------------------
# OPCIÓN 3: SHOCK IMPORTACIÓN (dic 2024)
# ------------------------------------------------------------
...
modelo_imp_m1 <- feols(                                                    
  ind_prod_nominal ~ Shock_Importacion:IVIG_1_dt +                         
                     cobertura +                                           
                     log(población) + log(SGP) |                           
                     departamento + ym,                                    
  data = BD_0,                                                             
...
modelo_imp_m2 <- feols(                                                    
  ind_prod_nominal ~ Shock_Importacion:IVIG_2_dt +                         
                     cobertura +                                           
                     log(población) + log(SGP) |                           
                     departamento + ym,                                    
  data = BD_0,                                                             
...
modelo_imp_m3 <- feols(                                                    
  ind_prod_nominal ~ Shock_Importacion:IVIG_3_dt +                         
                     cobertura +                                           
                     log(población) + log(SGP) |                           
                     departamento + ym,                                    
  data = BD_0,                                                             
...
cat("============================================================\n\n")              
etable(modelo_imp_m1, modelo_imp_m2, modelo_imp_m3,                                 
       title = "Efecto de la Importación de Gas (GNI) sobre Productividad Industrial", 
       tex = TRUE,                                                                   
       headers = c("M1","M2","M3"),                                                  
       dict = c("Shock_Importacion:IVIG_1_dt" = "Importación × Grupo Vulnerable (M1)",
                "Shock_Importacion:IVIG_2_dt" = "Importación × Grupo Vulnerable (M2)",
                "Shock_Importacion:IVIG_3_dt" = "Importación × Grupo Vulnerable (M3)",
                "ind_empleo" = "Empleo industrial",                                  
                "cobertura" = "Cobertura de gas",                                     
                "log(PIB_DPTO)" = "log(PIB departamental)",                           
                "log(población)" = "log(Población)",                                   
